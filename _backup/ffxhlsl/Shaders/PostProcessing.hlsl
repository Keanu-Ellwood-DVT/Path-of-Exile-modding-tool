...
{
	float dist = length(screen_coord - 0.5f);
	/*float falloff = 0.5f;
	float ratio = 1.0f / pow(pow(dist * falloff, 2.0f) + 1.0f, 2.0f);*/

	//float ratio = smoothstep(1.0f, 0.1f, dist/* * 0.5f*/);
	float ratio = pow(1.0f - pow(dist / (sqrt(2.0f) / 2.0f), 2.0f) * 1.0f, 1.0f);
	ratio = lerp(ratio, 1.0f, 0.15f);
...
{
	float2 delta = screen_coord - 0.5f;
	float dist_ratio = length(delta) / (sqrt(2.0f) / 2.0f);
	float4 sum_weight = 0.0f;
	float4 sum_color = 0.0f;
	int iterations_count = 2;
	float2 blur_x = normalize(delta);
	float2 blur_y = float2(-blur_x.y, blur_x.x);
	float2 blur_dir = blur_x * blur_intensity.x + blur_y * blur_intensity.y;
	float blur_dist = pow(length(delta), blur_curvature);// * (1.0f - rand(tex_coord) / float(iterations_count));
	float rand_offset = rand(screen_coord);

	float4 red_mult = float4(1.0f, 1.0f, 0.0f, 1.0f);
	float4 blue_mult = float4(0.0f, 1.0f, 1.0f, 1.0f);
	for(int i = 0; i < iterations_count; i++)
	{
		float blur_ratio = (float(i) + rand_offset) / float(iterations_count);
		float4 chromatic_mult = lerp(red_mult, blue_mult, blur_ratio);
		float4 weight = chromatic_mult;//float(iterations_count - i) / float(iterations_count);
		sum_weight += weight;
		float2 offset_coord = screen_coord - blur_dir * blur_dist * blur_ratio * 0.1f;
		sum_color += SAMPLE_TEX2DLOD( source_sampler, SamplerLinearClamp, float4(offset_coord * src_frame_to_dynamic_scale.xy, 0.0f, 0.0f) ) * weight;
	}
...
float4 PostProcessPS( PInput input ) : PIXEL_RETURN_SEMANTIC
{
	...
	#if DETAIL_LEVEL > 0
	{
		float3 bloom = SAMPLE_TEX2D( bloom_sampler, SamplerLinearClamp, screen_coord.xy ).rgb;
		colour.rgb = colour.rgb * original_intensity + bloom * colour.a; // keep the original alpha value, because we need it for screenshot taking
	}
	#endif
